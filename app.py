# app.py — Auto Detailing (Supabase + admin + emails)
...
# DETAILING TABLE DEFAULT
SUPABASE_TABLE = os.getenv("SUPABASE_TABLE", "appointments_detail").strip()
...
# ---------- Site loader (detailing; env-driven) ----------
def load_site() -> dict:
    hero_urls = [s.strip() for s in (os.getenv("HERO_IMAGES", "")).split(",") if s.strip()]
    return {
        "slug": "autodetail",
        "brand": {
            "name": os.getenv("SALON_NAME", "The Locals Detailing Co."),
            "tagline": os.getenv("TAGLINE", "Paint-safe detailing, ceramic coatings & interior deep cleans."),
        },
        "theme": {
            "gradient_start": os.getenv("GRADIENT_START", "#0ea5e9"),  # blue
            "gradient_end":   os.getenv("GRADIENT_END",   "#111827"),  # near-black
        },
        "hero": {
            "cta_text": os.getenv("CTA_TEXT", "Book Detailing"),
            "cta_url":  os.getenv("CTA_URL",  "/book"),
            "subtext":  os.getenv("HERO_SUBTEXT", "Shine that lasts. Mobile friendly, paint-safe techniques."),
            "images":   hero_urls,  # if empty, template will use static fallbacks
        },
        "services": [],  # can wire to DB later
        "reviews":  [],  # can wire to DB later
        "contact": {
            "address":   os.getenv("SALON_ADDRESS", "Chalfont, PA"),
...
def sb_slot_taken(date_str: str, time_str: str) -> bool:
    ...
    taken = bool(r.data)
    print(f"[BOOKING] slot_taken? date={date_str} time={time_str} -> {taken}")
    return taken


# REPLACE your old sb_insert_booking with this:
def sb_insert_booking(date_str, time_str, name, service, email):
    payload = {
        "date": date_str,
        "time": time_str,
        "name": name,
    ...
    if not getattr(res, "data", None):
        raise RuntimeError(f"Insert returned no data: {res}")
    return res.data
...
# ---------- Hours & slots (detailing defaults; override with HOURS env) ----------
def _parse_hours_env():
    """
    env HOURS like:
      HOURS="Mon=9:00 AM-6:00 PM;Tue=9:00 AM-6:00 PM;...;Sat=9:00 AM-4:00 PM"
    Returns dict weekday index (Mon=0..Sun=6) -> (start,end)
    """
    ...
    for part in raw.split(";"):
        ...
        di = mapping.get(day.strip()[:3].lower())
        if di is None or "-" not in span: continue
        start, end = [s.strip() for s in span.split("-", 1)]
        out[di] = (start, end)
    return out or None

DEFAULT_HOURS = {
    0: ("9:00 AM", "6:00 PM"),  # Mon
    1: ("9:00 AM", "6:00 PM"),  # Tue
    2: ("9:00 AM", "6:00 PM"),  # Wed
    3: ("9:00 AM", "6:00 PM"),  # Thu
    4: ("9:00 AM", "6:00 PM"),  # Fri
    5: ("9:00 AM", "4:00 PM"),  # Sat
    # Sun closed
}
...
def book():
    if request.method == "POST":
        ...
        try:
            # If the slot is already scheduled, stop early (unique index will also protect us)
            if sb_slot_taken(date_str, time_str):
                return "Time already booked", 400

            # Insert the booking (this now logs what’s happening)
            ...
            # Keep the user-facing message simple; details are in your terminal
            return "Server error — please try again.", 500

        # Render confirmation page
        return render_template(
            "confirmation.html",
            date=date_str, time=time_str, name=name, service=service
        )
...
def login_post():
    ...
    # Option 2: Supabase-backed admin (table: admins)
...
SALON_NAME = os.getenv("SALON_NAME", "The Locals Detailing Co.")
SALON_ADDRESS = os.getenv("SALON_ADDRESS", "Chalfont, PA")
...
def send_thanks_email(appt: dict) -> bool:
    ...
    if not email: return False
    name = appt.get("name") or "there"
    subject = f"Thanks for choosing {SALON_NAME}!"
    body = "\n".join([
        f"Hi {name},",
        "",
        "Thanks for coming in today — we hope you loved the results!",
        f"• {_fmt_appt_line(appt)}",
        "",
        "If there’s anything we can do better, just reply to this email.",
        "",
        "Drive clean, stay gleaming,",
        SALON_NAME,
        SALON_ADDRESS,
    ])
    return send_email(email, subject, body)
...
BOOK_BADGES = [s.strip() for s in os.getenv(
    "BOOK_BADGES", "Mobile service|Ceramic coatings|Pet-hair removal"
).split("|") if s.strip()]

@app.context_processor
def inject_brand_and_badges():
    return {
        "brand_name": os.getenv("SALON_NAME", "The Locals Detailing Co."),
        "badges": BOOK_BADGES,
